// ender-lilies-parser.js - النسخة المحدثة
export function parseEnderLiliesSaveData(fileContent) {
  try {
    ...
    // التحقق من البنية الأساسية للملف مع قيم افتراضية
    if (!saveData.root) {
      throw new Error('Save file missing root structure');
    }
    ...
    // استخراج المعلومات الأساسية مع قيم افتراضية
    ...
    // إرجاع البيانات مع تنظيف أي قيم undefined
    const result = {
      level: level || 1,
      exp: exp || 0,
      healPower: healPower || 1,
      currentHeals: currentHeals || 0,
      currencies: {
        spiritLv1: spiritLv1 || 0,
        spiritLv2: spiritLv2 || 0,
        spiritLv3: spiritLv3 || 0
      },
      spirits: (spirits || []).map(spiritId => ({
        id: spiritId || 'unknown',
        name: getSpiritName(spiritId),
        level: (spiritLevels.find(sl => sl.key?.Name === spiritId)?.value?.Int || 1)
      })).filter(spirit => spirit.id !== 'unknown'),
      passives: (passives || []).map(passiveId => ({
        id: passiveId || 'unknown',
        name: getPassiveName(passiveId)
      })).filter(passive => passive.id !== 'unknown'),
      openedAreas: (openedAreas || []).map(areaId => ({
        id: areaId || 'unknown',
        name: getAreaName(areaId)
      })).filter(area => area.id !== 'unknown'),
      achievements: (achievements || []).map(achievementId => ({
        id: achievementId || 'unknown',
        name: getAchievementName(achievementId)
      })).filter(achievement => achievement.id !== 'unknown'),
      playTime: playTimeHours || '0.0',
...
function cleanResult(obj) {
  ...
  // إزالة أي خصائص بقيمة null من المصفوفات الداخلية
  if (cleaned.spirits) {
    cleaned.spirits = cleaned.spirits.filter(spirit => spirit !== null);
  }
  if (cleaned.passives) {
    cleaned.passives = cleaned.passives.filter(passive => passive !== null);
  }
  if (cleaned.openedAreas) {
    cleaned.openedAreas = cleaned.openedAreas.filter(area => area !== null);
  }
  if (cleaned.achievements) {
    cleaned.achievements = cleaned.achievements.filter(achievement => achievement !== null);
  }
