...
window.loadGameAnalysis = async function(gameDocId) {
  try {
    ...
    if (gameData.gameId === 'ender-lilies' && gameData.analysisData) {
      console.log('Displaying Ender Lilies analysis');
      displayEnderLiliesAnalysis(gameData.analysisData, gameData);
    } 
    else if (gameData.gameId === 'hollow-knight' && gameData.analysisData) {
      console.log('Displaying Hollow Knight analysis');
      console.log('Hollow Knight analysis data structure:', gameData.analysisData);
      ...
      document.getElementById('game-analysis-content').innerHTML = `
        <div style="text-align: center; padding: 2rem;">
          ...
          <div style="margin-top: 1rem; padding: 1rem; background: var(--bg-color); border-radius: var(--border-radius);">
            <p style="font-size: 0.9rem; color: var(--text-secondary);">
              Debug Info:<br>
              Game ID: ${gameData.gameId}<br>
              Has Analysis Data: ${!!gameData.analysisData}<br>
...
function displayHollowKnightAnalysis(analysisData, gameData) {
  ...
  const abilities = analysisData.abilities || {};
  const charms = analysisData.charms || {};
  const bosses = analysisData.bosses || {};
  const collectibles = analysisData.collectibles || {};
  const questStatus = analysisData.questStatus || {};
  ...
  const content = `
    <div class="game-analysis">
      ...
      <div class="file-info" style="background: var(--bg-color); padding: 1rem; border-radius: var(--border-radius); margin-bottom: 2rem;">
        ...
        <p><strong>Uploaded:</strong> ${gameData.uploadedAt ? new Date(gameData.uploadedAt.toDate()).toLocaleString() : 'Unknown'}</p>
      </div>
...
function displayEnderLiliesAnalysis(analysisData, gameData) {
  // Calculate the total number of spirits from the data
  const totalSpirits = 26; // The total number of spirits in the game
      ...
      <div class="achievements-section">
        ...
        <div class="achievements-grid">
          ${analysisData.achievements.map(achievement => {
            // Add a special class for golden achievements
